<template>
  <section class="container" id="app">
    <h1 class="app-title">Gi<span>ff</span>y</h1>
    <Header
      @type="(v) => search(v)"
      @search="search"
      @limit="(v) => limit = v"
...
export default {
  ...
  async mounted () {
    this.gifs = await APICall('/trending', { limit: this.limit })
  },
  methods: { 
    async search (q = this.q) {
      this.gifs = await APICall('/search', { limit: this.limit, q })
    }
  }
}
</script>
